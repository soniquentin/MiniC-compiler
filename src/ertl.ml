...
let init_live_info (graph) = 
    ...
    in Label.M.iter init_solo graph
...
let liveness (graph) = 
  init_live_info(graph);
  modify_pred(live_info_tab);
  kildall();
