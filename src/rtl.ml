...
let rec translate_expr (e:Ast.texpr) (rd:register) (ld:label) (r_env:(string,register) Hashtbl.t) r_env_acc= match e with 
  ...
  | Ast.TEunop (uop, te) -> begin match uop with 
        | Ast.TUneg -> (*   L_{d-1} := Sub(#r_zero , #rd ) -> Ld
                      ...
                      translate_expr te rd l_d_2 r_env r_env_acc
        | Ast.TUnot -> let l_d_1 = add_label_in_graph (Emunop ((Ops.Msetei Int32.zero), rd, ld)) in translate_expr te rd l_d_1 r_env r_env_acc
      end
  | Ast.TEbinop (biop, te1, te2) -> begin match biop with
        ...
        | (Ast.TBadd|Ast.TBdiv|Ast.TBsub|Ast.TBeqq|Ast.TBneq|Ast.TBge|Ast.TBgt|Ast.TBle|Ast.TBlt) -> let r_temp = Register.fresh() in let l_d_1 = add_label_in_graph (Embinop  ( tast_to_ops biop, r_temp, rd, ld )) in
                                                                                                    let l_d_2 = translate_expr te2 rd l_d_1 r_env r_env_acc in
                                                                                                    translate_expr te1 r_temp l_d_2 r_env r_env_acc
        | Ast.TBmul (* Cas particulier : on veut simplifier. 0 x e = 0 et 1 x e = e *) -> begin match te1, te2 with
                ...
                | Ast.TEint 1l, _ -> translate_expr te2 rd ld r_env r_env_acc
                | _, Ast.TEint 1l -> translate_expr te1 rd ld r_env r_env_acc
        ...
        | Ast.TBand -> (* On va faire des égalités:
                        ...
                        let l_d_4 = translate_expr te1 r_temp l_d_3 r_env r_env_acc in
                        translate_expr te2 rd l_d_4 r_env r_env_acc
  ...
  | Ast.TEassign (t_assigned, te) -> begin match t_assigned with (* On va déjà checker que te est une valeur à gauche *)
      | Ast.TEident id -> begin try let r_id = Hashtbl.find r_env id in 
                            let l_d_1 = add_label_in_graph (Embinop (Ops.Mmov, r_id, rd, ld)) in translate_expr te r_id l_d_1 r_env r_env_acc
                        with Not_found -> raise (Error "Impossible de savoir la taille d'un objet de type null")
                      end
      | Ast.TEaccess (te_struct, field_x) -> 
                                      ...
                                      *)
                                      ...
                                      let l_d_3 = translate_expr te r_expr l_d_2 r_env r_env_acc in 
                                      let l_d_4 = translate_expr te_struct r_struct l_d_3 r_env r_env_acc in l_d_4
  ...
  | Ast.TEaccess (te, field_x) -> let pos = field_x.Ast.field_pos in let r_temp = Register.fresh() in
                                                        ...
                                                        let l_d_2 = translate_expr te r_temp l_d_1 r_env r_env_acc in l_d_2
                              ...
                              *)  
                              begin try let f_info = Hashtbl.find all_fun id in 
                                ...
                                let rec store_all_expr expr_list reg_list final_l = begin match expr_list, reg_list with 
                                    | [],[] -> final_l
                                    | e1::q1 , r2::q2 -> let next_l = store_all_expr q1 q2 final_l in translate_expr e1 r2 next_l r_env r_env_acc
...
and translate_instr (i:Ast.tdecl) (rd:register) (ld:label) (rret:register) (r_env:(string,register) Hashtbl.t) r_env_acc  = match i with
    | Ast.TDvar td_vars -> translate_decl_vars td_vars rd ld r_env r_env_acc
    | Ast.TDtyp td_typ -> add_label_in_graph (Egoto ld) (* Rien à faire *)
    | Ast.TDfct td_fct ->   translate_decl_fct td_fct rd ld r_env r_env_acc
    | Ast.TDecl_instr td_instr -> translate_decl_instr td_instr rd ld rret r_env r_env_acc
and translate_decl_instr (x:Ast.tdecl_instr) (rd:register) (ld:label) (rret:register) (r_env:(string,register) Hashtbl.t) r_env_acc = begin match x with
      | Ast.Tnone -> add_label_in_graph (Egoto ld)
      | Ast.Texpr te -> translate_expr te rd ld r_env r_env_acc
      | Ast.Tblock tb -> translate_block tb rd ld rret r_env r_env_acc
      | Ast.Tifelse (te, t_instr1, t_instr2) -> (* l_true := instruction t_instr1 -> Ld
                                                ...
                                                *)
                                                let l_true = translate_decl_instr t_instr1 rd ld rret r_env r_env_acc in
                                                let l_false = translate_decl_instr t_instr2 rd ld rret r_env r_env_acc  in 
                                                let r_temp = Register.fresh() in let l_d_2 = add_label_in_graph (Emubranch (Ops.Mjnz, r_temp, l_true, l_false)) in 
                                                translate_expr te r_temp l_d_2 r_env r_env_acc
      | Ast.Tif (te, t_instr) -> translate_decl_instr (Ast.Tifelse (te, t_instr, Ast.Tnone)) rd ld rret r_env r_env_acc
      | Ast.Twhile (te, t_instr) ->   let l_d_2  = Label.fresh() in let r_temp = Register.fresh() in
                                      let test_e = translate_expr te r_temp l_d_2 r_env r_env_acc in
                                      let l_true = translate_decl_instr t_instr rd l_d_2 rret r_env r_env_acc in
      ...
      | Tret (Some te) -> translate_expr te rret ld r_env r_env_acc 
    end
and translate_block (tb:Ast.tblock) (rd:register) (ld:label) (rret:register) (r_env:(string,register) Hashtbl.t) r_env_acc = let def_var = ref [] in
    let rec clean_vars_space var_list =  begin match var_list with (* Fonction pour retirer toutes les variables définies dans le block *)
      | [] -> ()
      | t :: q -> Hashtbl.remove r_env t; clean_vars_space q;
      end in
      let handle_def_var = function
          | Ast.TDvar tvars -> begin match tvars with
                                  | Ast.TDecl_solo (_,id,_) -> def_var := id :: !def_var; let r_id = Register.fresh() in Hashtbl.add r_env id r_id; Hashtbl.add r_env_acc id r_id
                                  | Ast.TDecl_multi(_,id_list) -> def_var := id_list @ !def_var; List.iter (fun x -> let r_id = Register.fresh() in Hashtbl.add r_env x r_id; Hashtbl.add r_env_acc x r_id) id_list
                              end
          | _ -> ()
      in List.iter (fun x -> handle_def_var x) tb ;
      let rec aux reverse_tdecl_list rd_next_instr ld_next_instr = begin match reverse_tdecl_list with (* On parcourt la liste d'instructions du block*) (* def_var track les variables définies*)
          | [] -> ld_next_instr, !def_var
          | Ast.TDecl_instr t :: q -> let r_temp = Register.fresh() in let l_entrance = translate_decl_instr t rd_next_instr ld_next_instr rret r_env r_env_acc in aux q r_temp l_entrance
          | Ast.TDvar t :: q -> let r_temp = Register.fresh() in let l_entrance = translate_decl_vars t rd_next_instr ld_next_instr r_env r_env_acc in aux q r_temp l_entrance
...
and translate_decl_vars (x:Ast.tdecl_vars) (rd:register) (ld:label) (r_env:(string,register) Hashtbl.t) r_env_acc = match x with
      | TDecl_solo (t, tid, te) -> let r_id = Hashtbl.find r_env tid in begin match t with
            (* POUR INT
              L_{d-1} := Mov r_id rd -> L_d
              L_{d-2} := te -> r_id     -> L_{d-1}
            *)
            | TInt ->  let l_d_1 = add_label_in_graph (Embinop (Ops.Mmov, r_id, rd, ld)) in translate_expr te r_id l_d_1 r_env r_env_acc
            | _ ->  raise (Error "Cannot declare something else than int")
          end
      | TDecl_multi (t, id_list) -> add_label_in_graph (Egoto ld)
and translate_decl_fct (x:Ast.tdecl_fct) (rd:register) (ld:label) (r_env:(string,register) Hashtbl.t) r_env_acc = let typ, id_fun, args_list, body = x in 
          let rec add_args_to_env accu = function (* Ajoute les arguments à l'environnement et renvoie la liste des pseudo-registres : chaque argument = registre frais *)
              | [] -> List.rev accu 
              | (_, id) :: q -> let arg_r = Register.fresh() in Hashtbl.add r_env id arg_r;Hashtbl.add r_env_acc id arg_r ; add_args_to_env (arg_r :: accu) q
          in let return_r = Register.fresh() in 
          let exit_label = Label.fresh() in
          let entry_label = translate_block body rd exit_label return_r r_env r_env_acc in 
          ...
          let f_info = {
            ...
            fun_locals = Register.set_of_list (extract_values r_env_acc);
...
and program (p:Ast.tfichier) = let r_env = (Hashtbl.create 17 : (string,register) Hashtbl.t) in let r_env_acc = (Hashtbl.create 17 : (string,register) Hashtbl.t) in let final_r = Register.fresh() in let final_l = Label.fresh() in
    let rec aux r_next l_next = function 
          | [] -> ()
          | t :: q -> let r_temp =  Register.fresh() in let prev_l = translate_instr t r_next l_next r_next r_env r_env_acc in aux r_temp prev_l q
